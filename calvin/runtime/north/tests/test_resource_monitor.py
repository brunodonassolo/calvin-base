...
class TestCpuMonitor(object):
    ...
    def test_avail_valid(self):
        ...
        for i in values:
            ...
            for j in range(0, values.index(i)):
                self.done = False
                self.storage.get_index(index=self.CPUAVAIL_INDEX_BASE + map(str, values[:j+1]), cb=CalvinCB(self.cb))
                yield wait_for(self._test_done)
    ...
    def test_total_valid(self):
        ...
        for i in values:
            ...
            for j in range(0, values.index(i)):
                self.done = False
                self.storage.get_index(index=self.CPUTOTAL_INDEX_BASE + map(str, values[:j+1]), cb=CalvinCB(self.cb))
                yield wait_for(self._test_done)
                assert self.node.id in self.get_ans
...
class TestMemMonitor(object):
    ...
    def test_avail_valid(self):
        ...
        for i in values:
            ...
            for j in range(0, values.index(i)):
                self.done = False
                self.storage.get_index(index=self.MEMAVAIL_INDEX_BASE + map(str, values[:j+1]), cb=CalvinCB(self.cb))
                yield wait_for(self._test_done)
                assert self.node.id in self.get_ans
    ...
    def test_total_valid(self):
        ...
        for i in values:
            ...
            for j in range(0, values.index(i)):
                self.done = False
                self.storage.get_index(index=self.MEMTOTAL_INDEX_BASE + map(str, values[:j+1]), cb=CalvinCB(self.cb))
                yield wait_for(self._test_done)
                assert self.node.id in self.get_ans
...
class TestLinkMonitor(object):
    ...
    def test_bandwidth_valid(self):
        ...
        for i in values:
            ...
            for j in range(0, values.index(i)):
                self.done = False
                self.storage.get_index(index=self.BANDWIDTH_INDEX_BASE + map(str, values[:j+1]), cb=CalvinCB(self.cb))
                yield wait_for(self._test_done)
    ...
    def test_stop_node(self):
        ...
        self.done = False
        self.storage.get_index(index=self.BANDWIDTH_INDEX_BASE + ['1M'], cb=CalvinCB(self.cb))
        yield wait_for(self._test_done)
        assert self.get_ans is None
